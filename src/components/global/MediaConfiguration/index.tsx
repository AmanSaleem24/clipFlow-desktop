...
const MediaConfiguration = ({ user, state }: Props) => {
  const activeScreenId = user?.studio?.screen || state.displays?.[0]?.id;
  const activeAudioId = user?.studio?.mic || state.audioInputs?.[0]?.deviceId;
  ...
  const activeAudio = state.audioInputs?.find(
    (device) => device.deviceId === activeAudioId,
  );
  ...
  return (
    <form className="flex h-full relative w-full flex-col gap-y-5">
      {isPending && (
        <div className="fixed z-50 w-full top-0 left-0 right-0 bottom-0 rounded-2xl h-full bg-black/8- flex">
          <Loader />
        </div>
      )}
      ...
      <div className="flex gap-x-5 justify-center items-center">
        ...
        >
          {state.audioInputs?.length ? (
            ...
            <option value="" className="bg-[#171717] cursor-pointer">
